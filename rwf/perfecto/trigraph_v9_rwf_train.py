...
class CFG:
    ...
    # Video stems that are genuine OOD failures (all segments FN despite good GQS).
    # Excluded from TRAINING only; val/test keep them for honest evaluation.
    # Source: error_analysis.csv — Fight_XM0NI7ZmwWM had 4/4 segments FN with
    # probs ≤0.08 despite q_int=q_obj=q_po=1.0, indicating extreme domain shift.
...
def train() -> None:
    ...
    # temp_anneal, label_smoothing}. Missing keys inherit cfg defaults.
    # Experiments not listed here run with all-default values — byte-identical
            ...
            try:
                ...
                for col in ("q_skel", "q_int", "q_po"):
                    if col not in test_sub.columns:
                        raise ValueError(
                            f"gqs_summary_v9.csv missing column {col}")
...
def _collect_outputs() -> list:
    ...
    TOP_FILES = ["experiment_comparison_v9.csv"]
    EXP_FILES = [
        "best.pt",
        "test_metrics.json",
        "train_history.csv",
...
def main() -> None:
    ...
    Usage:
        modal run trigraph_v9_rwf_train.py
    """
    print("Guardian Eye V9 — Phase 3: Training + auto-download")
    train.remote()
