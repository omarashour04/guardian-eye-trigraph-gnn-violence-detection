...
Usage
─────
  modal run trigraph_v9_rwf_train.py::train
  modal run trigraph_v9_rwf_train.py::results
"""
...
# ── Modal primitives ──────────────────────────────────────────────────────────
APP_NAME      = "guardian-eye-v9-rwf-train"
...
class CFG:
    ...
    pos_weight:      float = 1.0     # balanced dataset
    batch_size:      int   = 64
...
def build_model_components():
    ...
    class GATv2Layer(nn.Module):
        ...
        def forward(self, nodes: torch.Tensor,
            ...
            # Aggregate values using mean over heads
    ...
    class V9Model(nn.Module):
        ...
        fusion_mode: 'qgf' = QualityGatedFusion, 'lw' = LearnedWeightFusion.
        """
        def __init__(self,
...
def train() -> None:
    ...
    if not SPLIT_CSV.exists():
        raise RuntimeError("split_v9.csv not found. Run Phase 1 first.")
    ...
    # RWF-2000 has only train+val. If no test split exists, reuse val for test
    # so the evaluation pipeline still runs on a non-empty set.
    test_is_val = len(test_paths) == 0
    if test_is_val:
        print("  No test split found in split_v9.csv; using val as test.")
        test_paths, test_labels = val_paths, val_labels
    print(f"Train={len(train_paths)}  Val={len(val_paths)}  "
          f"Test={len(test_paths)}"
          + ("  (test=val fallback)" if test_is_val else ""))

    train_ds = RWFDataset(train_paths, train_labels)
    val_ds   = RWFDataset(val_paths,   val_labels)
    # Avoid duplicating in-memory storage when test mirrors val (fallback path)
    test_ds  = val_ds if test_is_val else RWFDataset(test_paths, test_labels)
    ...
    def smooth_bce(logits, targets):
        ...
        t     = targets * (1.0 - alpha) + (1.0 - targets) * alpha
    ...
    for exp_id, active_streams, fmode in EXPERIMENTS:
      try:
        ...
        # GQS stratification analysis: split test set by valid_ratio quartile
        # to demonstrate GQS value (most relevant for E-QGF vs E-LW comparison)
        gqs_df_path = PROC_MOUNT / "gqs_summary_v9.csv"
        if gqs_df_path.exists() and ("qgf" in exp_id or "lw" in exp_id):
            try:
                gqs_df  = pd.read_csv(str(gqs_df_path))
                # If no "test" split exists in GQS summary, fall back to val
                test_sub = gqs_df[gqs_df["split"] == "test"].copy()
                if len(test_sub) == 0:
                    test_sub = gqs_df[gqs_df["split"] == "val"].copy()
                if len(test_sub) == 0:
                    raise ValueError("no test/val rows in gqs_summary_v9.csv")
                test_sub["q_bucket"] = pd.qcut(
                    test_sub["valid_ratio"], q=4,
                    labels=["Q1_low","Q2","Q3","Q4_high"])
                # Map video_id to prediction
                vid_to_idx = {
                    test_ds.samples[i]["video_id"]: i
                    for i in range(len(test_ds))
                }
                quartile_rows = []
                for bucket, grp in test_sub.groupby("q_bucket"):
                    ...
                    quartile_rows.append({
                        "experiment": exp_id, "bucket": str(bucket),
                        "n": len(idxs), "macro_f1": f1q,
                    })
    ...
    results_csv = RUN_DIR / "experiment_comparison_v9.csv"
    results_df.to_csv(str(results_csv), index=False)

    print(f"\n{'='*70}")
    print("V9 ABLATION COMPARISON")
    print(results_df[["experiment", "test_macro_f1",
                       "test_roc_auc", "test_accuracy",
                       "test_threshold"]].to_string(index=False))
    print(f"{'='*70}")
    print("\nKey comparison — GQS ablation:")
    subset = results_df[results_df["experiment"].isin(
        ["E_full_qgf", "E_full_lw"])]
    if len(subset):
        print(subset[["experiment", "fusion",
                       "test_macro_f1", "test_roc_auc"]].to_string(
            index=False))
    vol_proc.commit()
...
def results() -> None:
    ...
    cmp = RUN_DIR / "experiment_comparison_v9.csv"
    if cmp.exists():
        df = pd.read_csv(str(cmp))
        print("\n=== V9 EXPERIMENT COMPARISON ===")
        print(df[["experiment", "fusion", "params",
                  "test_macro_f1", "test_roc_auc",
                  "test_accuracy", "test_threshold"]].to_string(index=False))
    else:
        print("No results yet. Run: modal run trigraph_v9_rwf_train.py::train")
...
def main() -> None:
    print("Guardian Eye V9 — Phase 3: Training")
    print("Run: modal run trigraph_v9_rwf_train.py::train")
    print("     modal run trigraph_v9_rwf_train.py::results")
