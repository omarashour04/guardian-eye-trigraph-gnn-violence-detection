...
Runs all four diagnostics in one command and downloads results locally:
  ...
  conda run -n dlcuda128 modal run trigraph_v9_ubi_diagnostics.py
...
Output files land in ./diagnostics_output/ on your machine:
  ...
  error_analysis.csv
  inference_speed.csv / .json
  shortcut_audit.csv            ← Phase-0 gate: prior shift + quality shortcut

Individual functions can still be called separately if needed:
  conda run -n dlcuda128 modal run trigraph_v9_ubi_diagnostics.py::gate_weights
  conda run -n dlcuda128 modal run trigraph_v9_ubi_diagnostics.py::gqs_stratified
  conda run -n dlcuda128 modal run trigraph_v9_ubi_diagnostics.py::calibration
  conda run -n dlcuda128 modal run trigraph_v9_ubi_diagnostics.py::error_analysis
  conda run -n dlcuda128 modal run trigraph_v9_ubi_diagnostics.py::shortcut_audit
"""
...
# ── Modal primitives ──────────────────────────────────────────────────────────
APP_NAME      = "guardian-eye-v9-ubi-diagnostics"
...
# ── Container image (identical pip_install block to trigraph_v9_ubi_train.py) ─
image = (
...
# ══════════════════════════════════════════════════════════════════════════════
# Configuration  (verbatim copy from trigraph_v9_ubi_train.py)
# ══════════════════════════════════════════════════════════════════════════════
@dataclass
class CFG:
...
# ══════════════════════════════════════════════════════════════════════════════
# Dataset  (verbatim copy from trigraph_v9_ubi_train.py)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# Model components  (verbatim copy from trigraph_v9_ubi_train.py,
...
def build_model_components():
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # ContentGatedFusion (ablation E_full_cgf) — inference-only copy
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # QualityGatedFusionAdv (ablation E_full_qgf_adv) — inference-only copy
    # ─────────────────────────────────────────────────────────────────────────
...
def _load_split(split_name: str, split_df):
    """Return (npz_paths, labels) for the requested split.
    UBI-Fights has an official test split — no val fallback.
    UBI split CSV uses clip_id (NOT video_id — that is RWF)."""
...
def _build_model_for_exp(exp_id: str, V9Model, device):
    ...
    EXP_CFG = {
        ...
        # Phase-2b ablations (2026-05-29)
        "E_full_cgf":     (("skeleton", "interaction", "object", "vit"), "cgf",  {}),
        "E_full_qgf_adv": (("skeleton", "interaction", "object", "vit"), "qgf_adv",
                           {"adv_weight": 0.1, "adv_lam": 1.0}),
    }
...
def gqs_stratified() -> None:
    ...
    if len(test_gqs) == 0:
        raise RuntimeError("No test/val rows in gqs_summary_ubi.csv")
    ...
    for col in ("q_skel", "q_int", "q_po"):
        if col not in test_gqs.columns:
            raise ValueError(f"gqs_summary_ubi.csv missing column {col}")
    ...
    # not stored in runs_ubi/).
    cmp_csv = RUN_DIR / "experiment_comparison_ubi.csv"
    ...
    if cmp_csv.exists():
        ...
        print("  WARN: experiment_comparison_ubi.csv not found; "
              "delta columns will be NaN.")
...
def error_analysis() -> None:
    ...
    if not gqs_path.exists():
        raise FileNotFoundError(f"GQS summary not found: {gqs_path}")
    gqs_df     = pd.read_csv(str(gqs_path))
    # UBI GQS CSV uses clip_id; dataset stores path.stem as "video_id" in
    # sample dict, but path.stem == clip_id for UBI (clips named by clip_id).
    gqs_lookup = gqs_df.set_index("clip_id")
...
# ══════════════════════════════════════════════════════════════════════════════
# FUNCTION 6: shortcut_audit
# Read-only data audit — NO model load, NO GPU work. Confirms the two
# distribution-shift failure modes directly from the CSV artifacts on the volume:
#   (1) prior shift  — positive rate per split (train/val should be ~0.3/0.21,
#       test ~0.62 on UBI-Fights).
#   (2) quality shortcut — AUC of each quality score ALONE at predicting the
#       fight label, per split. If train AUC >> test AUC, the model has a
#       train-only "high quality => fight" shortcut that does not transfer.
# Emits shortcut_audit.csv. This is the Phase-0 gate before any training change.
# ══════════════════════════════════════════════════════════════════════════════
@app.function(**_FUNC_KWARGS)
def shortcut_audit() -> None:
    import numpy as np
    import pandas as pd
    ...
    # GQS summary already carries split + label + per-stream quality scores, so
    # no NPZ load or model is needed. UBI CSVs key on clip_id (NOT video_id).
    gqs_path = PROC_MOUNT / "gqs_summary_ubi.csv"
    if not gqs_path.exists():
        raise FileNotFoundError(f"GQS summary not found: {gqs_path}")
    g = pd.read_csv(str(gqs_path))

    # If the GQS summary lacks split/label, merge them from the split CSV.
    if "split" not in g.columns or "label" not in g.columns:
        split_df = pd.read_csv(str(SPLIT_CSV))[["clip_id", "split", "label"]]
        g = g.merge(split_df, on="clip_id", how="left", suffixes=("", "_s"))

    # Composite mirrors the formula used in train.py / gqs_stratified
    # (0.4 q_skel + 0.4 q_int + 0.2 q_po).
    g["gqs_composite"] = (
        0.4 * g["q_skel"] + 0.4 * g["q_int"] + 0.2 * g["q_po"]
    )

    quality_cols = ["q_skel", "q_int", "q_obj", "q_po",
                    "valid_ratio", "gqs_composite"]
    quality_cols = [c for c in quality_cols if c in g.columns]

    rows = []
    print("\n=== Shortcut audit — per split ===")
    for sp in ["train", "val", "test"]:
        ...
        print(f"\n  {sp}: n={n}  pos={n_pos}  neg={n_neg}  "
              f"pos_rate={pos_rate:.4f}")
        row = {"split": sp, "n": n, "n_pos": n_pos, "n_neg": n_neg,
               "pos_rate": round(pos_rate, 4)}
        # AUC of each quality score ALONE at predicting the fight label.
        for col in quality_cols:
            try:
                auc = float(roc_auc_score(sub["label"], sub[col]))
            except ValueError:
                auc = float("nan")   # single-class split
            row[f"auc_{col}"] = round(auc, 4)
            print(f"    quality-alone AUC  {col:<14}: {auc:.4f}")
        rows.append(row)

    audit_df = pd.DataFrame(rows)

    # Headline deltas: prior gap (test vs train) and shortcut gap (train vs test
    # composite AUC). Large positive values on both confirm the diagnosis.
    try:
        tr = audit_df.set_index("split").loc["train"]
        te = audit_df.set_index("split").loc["test"]
        prior_gap    = float(te["pos_rate"] - tr["pos_rate"])
        shortcut_gap = float(tr.get("auc_gqs_composite", float("nan"))
                             - te.get("auc_gqs_composite", float("nan")))
        print("\n=== Headline ===")
        print(f"  prior gap (test pos_rate - train pos_rate): {prior_gap:+.4f}")
        print(f"  shortcut gap (train - test composite AUC) : {shortcut_gap:+.4f}")
        print("  > prior gap >> 0 confirms inverted base rate.")
        print("  > shortcut gap >> 0 confirms non-transferable quality shortcut.")
    except Exception as e:
        print(f"  WARN: could not compute headline deltas: {e}")

    out_path = DIAG_DIR / "shortcut_audit.csv"
    audit_df.to_csv(str(out_path), index=False)
    print(f"\n  Saved: {out_path}")

    vol_proc.commit()
    print("shortcut_audit complete.")


# ══════════════════════════════════════════════════════════════════════════════
# FUNCTION 6b: embedding_shortcut
#   Answers the question the gate/sampler fixes left open: is the quality
#   shortcut frozen INTO the VideoMAE embeddings (upstream of the graph head),
#   rather than in the gate or the prior?
#
#   Two linear probes on the RAW 768-d vit_embedding (no graph, no gate):
#     (A) embedding -> q_skel-high?   (does the backbone encode skeleton quality?)
#     (B) embedding -> label           fit on TRAIN only, scored on train/val/test
#         (if val AUC >> test AUC, the label-shortcut is baked into the embedding
#          and no amount of head-side reweighting can transfer it)
#   Plus val/test quality-distribution overlap (mean q_skel for pos vs neg).
#   Read-only: reads NPZ vit_embedding + gqs_summary; trains tiny sklearn probes.
# ══════════════════════════════════════════════════════════════════════════════
@app.function(**_FUNC_KWARGS)
def embedding_shortcut() -> None:
    import numpy as np
    import pandas as pd
    ...
    split_df = pd.read_csv(str(SPLIT_CSV))           # clip_id, split, label
    gqs_path = PROC_MOUNT / "gqs_summary_ubi.csv"
    g = pd.read_csv(str(gqs_path)).set_index("clip_id")  # per-clip quality

    # ── Load raw vit_embedding for every split, straight from NPZ ──────────────
    def load_split_emb(sp: str):
        ...
        for cid, lab in tqdm(zip(sub["clip_id"], sub["label"]),
            ...
            y.append(int(lab))
            # q_skel from the summary (same clip key); fall back to NaN
            qsk.append(float(g["q_skel"].get(cid, np.nan))
                       if "q_skel" in g.columns else np.nan)
        return (np.stack(X) if X else np.zeros((0, cfg.vit_embed_dim)),
                np.array(y), np.array(qsk, dtype=np.float32))

    print("\n=== Loading raw VideoMAE embeddings per split ===")
    Xtr, ytr, qtr = load_split_emb("train")
    Xva, yva, qva = load_split_emb("val")
    Xte, yte, qte = load_split_emb("test")
    print(f"  train {Xtr.shape}  val {Xva.shape}  test {Xte.shape}")

    rows = []

    # ── Probe A: embedding -> high-quality-skeleton?  (per split, in-split CV) ──
    # Binarize q_skel at the train median so "high" is well-defined globally.
    if not np.all(np.isnan(qtr)) and Xtr.shape[0] > 0:
        thr_q = float(np.nanmedian(qtr))
        print(f"\n=== Probe A: does the embedding encode skeleton quality? "
              f"(q_skel split at train median {thr_q:.3f}) ===")
        sc_q = StandardScaler().fit(Xtr)
        probe_q = LogisticRegression(max_iter=2000, C=1.0)
        # Fit on train (embedding -> q_skel-high), score in-split per split.
        qtr_hi = (qtr >= thr_q).astype(int)
        if len(np.unique(qtr_hi)) == 2:
            probe_q.fit(sc_q.transform(Xtr), qtr_hi)
            for sp, X, q in [("train", Xtr, qtr), ("val", Xva, qva),
                             ("test", Xte, qte)]:
                if X.shape[0] == 0:
                    continue
                q_hi = (q >= thr_q).astype(int)
                if len(np.unique(q_hi)) < 2:
                    auc = float("nan")
                else:
                    auc = float(roc_auc_score(
                        q_hi, probe_q.predict_proba(sc_q.transform(X))[:, 1]))
                print(f"  {sp:<5} embedding->q_skel-high  AUC = {auc:.4f}")
                rows.append({"probe": "emb->q_skel_high", "split": sp,
                             "auc": round(auc, 4)})
        print("  > high AUC means the FROZEN embedding carries skeleton-quality "
              "info, so any quality<->label correlation rides along with it.")

    # ── Probe B: embedding -> label, fit on TRAIN, scored on every split ───────
    print("\n=== Probe B: linear label-probe on raw embedding "
          "(fit TRAIN, transfer to val/test) ===")
    sc = StandardScaler().fit(Xtr)
    probe = LogisticRegression(max_iter=2000, C=1.0, class_weight="balanced")
    probe.fit(sc.transform(Xtr), ytr)
    auc_by_split = {}
    for sp, X, y in [("train", Xtr, ytr), ("val", Xva, yva), ("test", Xte, yte)]:
        if X.shape[0] == 0 or len(np.unique(y)) < 2:
            auc = float("nan")
        else:
            auc = float(roc_auc_score(
                y, probe.predict_proba(sc.transform(X))[:, 1]))
        auc_by_split[sp] = auc
        print(f"  {sp:<5} embedding->label  AUC = {auc:.4f}")
        rows.append({"probe": "emb->label", "split": sp, "auc": round(auc, 4)})

    val_test_gap = auc_by_split.get("val", float("nan")) - \
        auc_by_split.get("test", float("nan"))
    print(f"\n  val-test embedding-probe AUC gap = {val_test_gap:+.4f}")
    print("  > a large positive gap proves the val->test collapse is INSIDE the "
          "frozen embeddings, upstream of the graph head — head-side fixes "
          "(sampler/prior/gate) cannot recover it.")

    # ── Quality distribution overlap: mean q_skel for pos vs neg, per split ────
    print("\n=== Quality distribution by label (mean q_skel) ===")
    for sp, y, q in [("train", ytr, qtr), ("val", yva, qva), ("test", yte, qte)]:
        if len(y) == 0:
            continue
        mp = float(np.nanmean(q[y == 1])) if (y == 1).any() else float("nan")
        mn = float(np.nanmean(q[y == 0])) if (y == 0).any() else float("nan")
        print(f"  {sp:<5} q_skel  pos={mp:.3f}  neg={mn:.3f}  "
              f"contrast(pos-neg)={mp - mn:+.3f}")
        rows.append({"probe": "qskel_mean_pos", "split": sp, "auc": round(mp, 4)})
        rows.append({"probe": "qskel_mean_neg", "split": sp, "auc": round(mn, 4)})

    out_path = DIAG_DIR / "embedding_shortcut.csv"
    pd.DataFrame(rows).to_csv(str(out_path), index=False)
    print(f"\n  Saved: {out_path}")

    vol_proc.commit()
    print("embedding_shortcut complete.")


@app.local_entrypoint()
def download_embedding_shortcut() -> None:
    """
    Run embedding_shortcut on Modal (T4) then download embedding_shortcut.csv.

    Usage:
        conda run -n dlcuda128 modal run \
            trigraph_v9_ubi_diagnostics.py::download_embedding_shortcut
    """
    import sys
    ...
    print("Running embedding_shortcut on Modal ...")
    embedding_shortcut.remote()

    rel_path = "runs_ubi/diagnostics/embedding_shortcut.csv"
    try:
        data = b"".join(vol_proc.read_file(rel_path))
        dest = LOCAL_OUT / "embedding_shortcut.csv"
...
# ══════════════════════════════════════════════════════════════════════════════
# FUNCTION 7: run_all  — run all diagnostics in one container invocation
# ══════════════════════════════════════════════════════════════════════════════
@app.function(**_FUNC_KWARGS)
def run_all() -> None:
    """
    Run gate_weights, gqs_stratified, calibration, error_analysis,
    inference_speed, and shortcut_audit sequentially inside a single Modal
    container. Outputs accumulate in /data/proc/runs_ubi/diagnostics/ on the
    volume.
    """
    print("=" * 70)
    print("STEP 1/6 — gate_weights")
    print("=" * 70)
    gate_weights.local()

    print("=" * 70)
    print("STEP 2/6 — gqs_stratified")
    print("=" * 70)
    gqs_stratified.local()

    print("=" * 70)
    print("STEP 3/6 — calibration")
    print("=" * 70)
    calibration.local()

    print("=" * 70)
    print("STEP 4/6 — error_analysis")
    print("=" * 70)
    error_analysis.local()

    print("=" * 70)
    print("STEP 5/6 — inference_speed")
    print("=" * 70)
    inference_speed.local()

    print("=" * 70)
    print("STEP 6/6 — shortcut_audit")
    print("=" * 70)
    shortcut_audit.local()

    print("=" * 70)
    print("All diagnostics complete. Results in /data/proc/runs_ubi/diagnostics/")
    print("=" * 70)
...
def main() -> None:
    ...
    Usage:
        modal run trigraph_v9_ubi_diagnostics.py
    """
    import sys
    from pathlib import Path as LocalPath
    ...
    EXPECTED = [
        "runs_ubi/diagnostics/gate_weights_E_full_qgf.csv",
        "runs_ubi/diagnostics/gqs_stratified_comparison.csv",
        "runs_ubi/diagnostics/calibration_results.json",
        "runs_ubi/diagnostics/error_analysis.csv",
        "runs_ubi/diagnostics/inference_speed.csv",
        "runs_ubi/diagnostics/inference_speed.json",
        "runs_ubi/diagnostics/shortcut_audit.csv",
    ]

    print("Guardian Eye V9 — UBI-Fights running all diagnostics on Modal (T4)...")
...
@app.local_entrypoint()
def download_shortcut_audit() -> None:
    """
    Run shortcut_audit on Modal (no GPU needed — just reads CSVs from volume)
    then download shortcut_audit.csv to ./diagnostics_output/.

    Usage:
        conda run -n dlcuda128 modal run trigraph_v9_ubi_diagnostics.py::download_shortcut_audit
    """
    import sys
    from pathlib import Path as LocalPath
    ...
    print("Running shortcut_audit on Modal ...")
    shortcut_audit.remote()

    rel_path = "runs_ubi/diagnostics/shortcut_audit.csv"
    try:
        data = b"".join(vol_proc.read_file(rel_path))
        dest = LOCAL_OUT / "shortcut_audit.csv"
        dest.write_bytes(data)
        print(f"  [ok] {dest.resolve()}")
    except Exception as e:
        print(f"  [FAIL] could not download {rel_path}: {e}")
