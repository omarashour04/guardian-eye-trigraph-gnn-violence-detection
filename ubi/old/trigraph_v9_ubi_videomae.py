...
def run_videomae(skip_finetune: bool = True) -> None:
    """
    Stage A: fine-tune VideoMAE-Base on UBI-Fights (skipped by default).
    ...
    if skip_finetune:
        ...
        # WeightedRandomSampler: each positive drawn ~pw_val× more often
        # so every batch sees a balanced mix regardless of dataset ratio.
        from torch.utils.data import WeightedRandomSampler
        sample_weights = torch.tensor(
            [pw_val if l == 1 else 1.0 for l in train_labels],
            dtype=torch.float64,
        )
