...
_cal_temp:     float = 1.0
_v9_ckpt_path: str | None = None


def load_v9_model(ckpt_path: Optional[str] = None) -> None:
    """
    Load the V9 E_full_qgf checkpoint into the module singleton.
    Call once at FastAPI startup when GUARDIAN_MOCK=0.
    """
    global _v9_model, _v9_threshold, _cal_temp, _v9_ckpt_path

    requested = ckpt_path or os.getenv("GUARDIAN_V9_CKPT", "")
    if not requested or not Path(requested).exists():
        raise RuntimeError(
    ...
    if _v9_model is not None and _v9_ckpt_path == resolved:
        return
    ...
    print(f"[classifier] Loading checkpoint: {resolved}")
    ...
    _v9_model = model
    _v9_ckpt_path = resolved
    print(
        f"[classifier] Ready  checkpoint={Path(resolved).name}  "
        f"threshold={_v9_threshold:.3f}  cal_temp={_cal_temp:.3f}"
    )
...
def _build_batch(d: "np.lib.npyio.NpzFile") -> dict:
    ...
    if "vit_embedding" in d:
        vit_np = d["vit_embedding"].astype(np.float32)
        vit_emb = torch.from_numpy(vit_np)
    else:
        vit_np = np.zeros(768, dtype=np.float32)
        vit_emb = torch.zeros(768, dtype=torch.float32)
    print(
        "[classifier][VideoMAE] npz_embedding "
        f"present={'vit_embedding' in d.files} "
        f"shape={tuple(vit_np.shape)} "
        f"mean={float(vit_np.mean()):.6f} "
        f"std={float(vit_np.std()):.6f} "
        f"all_zero={bool(np.allclose(vit_np, 0.0))}"
    )
...
def _modality_activity(d: "np.lib.npyio.NpzFile", gate_dict: dict[str, float]) -> dict:
    """
    Detect whether each stream has real signal in the cached NPZ.
    This is CPU-only metadata; it does not affect logits, gates, or verdict.
    """
    activity = {
        "skeleton": _array_active(d, "skeleton"),
        "interaction": (
            _array_active(d, "int_node_mask")
            or _array_active(d, "int_edge_mask")
            or _array_active(d, "int_edges")
        ),
        "object": (
            _array_active(d, "obj_node_mask")
            or _array_active(d, "po_edge_mask")
            or _array_active(d, "po_edges")
        ),
        "vit": _array_active(d, "vit_embedding"),
    }
    active = [name for name, is_active in activity.items() if is_active]
    inactive = [name for name, is_active in activity.items() if not is_active]
    raw_gate_sum = float(sum(float(value) for value in gate_dict.values()))
    inactive_gate_sum = float(sum(float(gate_dict.get(name, 0.0)) for name in inactive))
    active_gate_sum = float(sum(float(gate_dict.get(name, 0.0)) for name in active))
    if inactive:
        status = "partial"
        message = (
            "One or more modality streams were missing or zero-filled. Raw gate "
            "weights are preserved, but inactive stream percentages should not be "
            "treated as valid evidence contributions."
        )
    else:
        status = "valid"
        message = "All expected modality streams contained non-zero cached inputs."
    return {
        "active_modalities": active,
        "inactive_modalities": inactive,
        "gate_validity": {
            "status": status,
            "message": message,
            "raw_gate_sum": raw_gate_sum,
            "active_gate_sum": active_gate_sum,
            "inactive_gate_sum": inactive_gate_sum,
            "unavailable_contributions": inactive,
        },
    }


def _array_active(d: "np.lib.npyio.NpzFile", key: str) -> bool:
    ...
    if arr.dtype == np.bool_:
        return bool(arr.any())
    return not bool(np.allclose(arr, 0.0))
...
def classifier_forward(
    ...
    *,
    ...
    npz_mtime = npz_file.stat().st_mtime if npz_file.exists() else None
    print(
        "[classifier] loading NPZ "
        f"npz_path={npz_path!r} "
        f"mtime={npz_mtime} "
        f"size_bytes={npz_size} "
        f"clip_id={clip_id!r}"
    )
    ...
    modality_metadata = _modality_activity(d, gate_dict)
    ...
    if modality_metadata["inactive_modalities"]:
        print(
            "[classifier][gate-validity] "
            f"inactive={modality_metadata['inactive_modalities']} "
            f"active={modality_metadata['active_modalities']} "
            f"inactive_gate_sum={modality_metadata['gate_validity']['inactive_gate_sum']:.3f}"
        )
    print(
        "[classifier] result "
        f"npz_path={npz_path!r} "
        f"clip_id={clip_id!r} "
        f"confidence={conf} "
        f"people={telemetry['people']} "
        f"peak_window={telemetry['peak_window']}"
    )
