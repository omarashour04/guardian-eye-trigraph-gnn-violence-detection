...
Stage A — Fine-tune VideoMAE-Base (ViT-Base/16, pretrained on Kinetics-400)
  as a standalone binary violence classifier on Hockey Fights.
...
Stage B — Embedding extraction.
  Load the best checkpoint. Run inference over all 1,000 clips (all splits).
...
──────────────────
  ...
  Mixup alpha=0.4 + CutMix alpha=0.4 (moderate aug — HF is small, 700-sample train)
...
Hockey Fights is 1:1 balanced so pos_weight ~1.0, computed dynamically.
...
────────────
  ...
  - Stage B skips NPZs that already have a vit_embedding.
  Just re-run the same command after any interruption.

Usage
─────
  python trigraph_v9_hf_videomae.py
  python trigraph_v9_hf_videomae.py --skip-finetune    # Stage B only
  python trigraph_v9_hf_videomae.py --force-finetune   # redo Stage A
  python trigraph_v9_hf_videomae.py --force-reembed    # redo all of Stage B
"""
...
OUTPUT_DIR  = Path(r"C:\Violence detection\ashour\fresh\HF\finetune_output")

CACHE_DIR   = PREPROC_DIR / "cache_v9"
SPLIT_CSV   = PREPROC_DIR / "split_hf.csv"
VIT_CKPT    = OUTPUT_DIR  / "videomae_best.pt"
VIT_LAST    = OUTPUT_DIR  / "videomae_last.pt"
VIT_LOG_CSV = OUTPUT_DIR  / "videomae_train_log.csv"
VIT_RESULTS = OUTPUT_DIR  / "videomae_test_results.json"
STAGEA_DONE = OUTPUT_DIR  / "videomae_stageA.done"
...
class ViTCFG:
    ...
    # Moderate mixup/cutmix: HF is small (700 train) — heavy aug can hurt
    mixup_alpha:     float = 0.4
    cutmix_alpha:    float = 0.4
    dropout:         float = 0.1
    ema_decay:       float = 0.999

    num_workers:     int   = 0   # Windows multiprocessing spawn
    seed:            int   = 42
...
class HFVideoDataset(Dataset):
    ...
    Augmentation applied on-the-fly so each epoch sees different crops/flips.
    """
...
def main(args) -> None:
    ...
    if skip_stage_a:
        ...
        train_ds = HFVideoDataset(train_paths, train_labels, augment=True)
        val_ds   = HFVideoDataset(val_paths,   val_labels,   augment=False)

        # Dynamic pos_weight — HF is 1:1 so pw ~1.0
        ...
        for epoch in range(start_epoch, vcfg.epochs + 1):
            ...
            # Atomic full-state write every epoch (resume point)
        ...
        test_ds     = HFVideoDataset(test_paths, test_labels, augment=False)
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # Stage B — Embedding extraction for ALL splits (train + val + test)
    # ═══════════════════════════════════════════════════════════════════════
    ...
    class LazyNPZDataset(Dataset):
        """Reads NPZs from disk on demand — avoids loading all 1,000 clips at once."""
        ...
        for batch in tqdm(full_loader, desc="Extracting+writing embeddings",
            ...
            for path_str, emb in zip(batch["paths"], embs):
                ...
                try:
                    ...
                    tmp.replace(npz_path)  # atomic on Windows
                    written += 1
                except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Hockey Fights V9 VideoMAE")
