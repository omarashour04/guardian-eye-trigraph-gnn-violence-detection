...
Head      : Linear(128->128) -> ReLU -> Dropout -> Linear(128->1)
...
Hockey Fights is 1:1 balanced (500 fight / 500 non-fight).
pos_weight computed dynamically from train labels (expect ~1.0).
No hard-negative exclusion needed (no known OOD clips in HF).

Note on PO stream
─────────────────
V8.1 results showed the PO stream increased false positives on Hockey Fights,
and object GQS mean was ~0.111 (near-empty signal on most clips — ice rink
footage has almost no relevant objects). Watch gate_weights in diagnostics;
if w_obj is consistently high and driving FPs, consider D_skel_int_obj as
the primary reported model rather than E_full_qgf.

Resumability
────────────
  - Finished experiments (those with test_metrics.json) are skipped.
  - An experiment killed mid-training writes last.pt every epoch.
  - Re-running the same command resumes automatically.
  Use --no-resume to force retrain from scratch.

Usage
─────
  python trigraph_v9_hf_train.py
  python trigraph_v9_hf_train.py --no-resume
"""
...
# ── Paths ─────────────────────────────────────────────────────────────────────
PREPROC_DIR  = Path(r"C:\Violence detection\ashour\fresh\HF\preproc_output")
...
SPLIT_CSV = PREPROC_DIR  / "split_hf.csv"
GQS_CSV   = PREPROC_DIR  / "gqs_summary_hf.csv"
RUN_DIR   = OUTPUT_DIR   / "runs_hf"
...
class CFG:
    ...
    # pos_weight computed dynamically (HF is 1:1 balanced -> expect ~1.0)
    batch_size:      int   = 64
    num_workers:     int   = 0
    epochs:          int   = 60
    patience:        int   = 12
...
class HFDataset(Dataset):
    ...
    clip_id column used (not video_id — that is RWF convention).
    """
...
def collate_v9(batch):
    """Default collate (all tensors same shape — no padding needed)."""
    keys_tensor = [
        "skeleton", "int_nodes", "int_edges", "int_node_mask", "int_edge_mask",
        "obj_nodes", "obj_node_mask", "po_edges", "po_edge_mask",
        "gqs", "vit_embedding", "label",
    ]
...
class GATv2Layer(nn.Module):
    ...
    def forward(self, nodes: torch.Tensor,
        ...
        B, M, _ = nodes.shape
        Q = self.Wq(nodes); K = self.Wk(nodes)
        V = self.Wv(nodes); E = self.We(edges)
        ...
        a = F.softmax(a, dim=2)
        # Mean over heads (not per-head einsum — GATv2 canonical pooling)
        V_heads = V.view(B, M, self.heads, self.d)
        agg = torch.einsum('bimh,bmhd->bihd', a, V_heads).reshape(B, M, -1)
...
def main(args) -> None:
    ...
    if not SPLIT_CSV.exists():
        raise RuntimeError(
            f"split_hf.csv not found at {SPLIT_CSV}. "
            "Run Phase 1 (trigraph_v9_hf_preprocess.py) first."
        )
    ...
    if len(test_paths) == 0:
        raise RuntimeError(
            "No test split found in split_hf.csv. "
            "Check that Phase 1 completed successfully."
        )
    print(f"Train={len(train_paths)}  Val={len(val_paths)}  "
          f"Test={len(test_paths)}")

    train_ds = HFDataset(train_paths, train_labels)
    val_ds   = HFDataset(val_paths,   val_labels)
    test_ds  = HFDataset(test_paths,  test_labels)

    # Dynamic pos_weight — HF is 1:1 so ~1.0
    ...
    # Read tensor dims from one sample
    s0     = train_ds[0]
    ...
    EXPERIMENTS = [
        ...
        ("E_full_lw",          ("skeleton", "interaction", "object", "vit"),
         "lw",  {}),
        # QGF with entropy regularisation + temperature annealing (2.0->0.5)
    ...
    for exp_id, active_streams, fmode, ovr in EXPERIMENTS:
        try:
            ...
            for epoch in range(start_epoch, cfg.epochs + 1):
                ...
                # Atomic full-state write every epoch
            ...
            # GQS stratification (for E_full variants)
    ...
    results_csv = RUN_DIR / "experiment_comparison_hf.csv"
    results_df.to_csv(str(results_csv), index=False)

    print(f"\n{'='*70}")
    print("HOCKEY FIGHTS V9 ABLATION COMPARISON")
    ...
    print(f"\nV8.1 reference: Macro-F1=0.930  ROC-AUC=0.9642 (E_full_trigraph)")
    print(f"V9 target     : Macro-F1>=0.930  ROC-AUC>=0.96")
    print(f"{'='*70}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Hockey Fights V9 Training")
