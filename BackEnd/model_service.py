...
# ── Real predict stub (Phase 4) ───────────────────────────────────────────────

def _real_predict(video_path: str, clip_id: str) -> dict:
    """
    TODO (Phase 4): Wire the real ST-HGT classifier here.
    Steps:
      1. Run preprocessing (YOLO11x-pose → skeleton NPZ, interaction NPZ, object NPZ).
      2. Load ST-HGT Stage G2 checkpoint.
      3. Forward with return_gates=True → verdict, confidence, gate[4], gqs[5].
      4. Build telemetry from int_edges geometry (peak window, weapon proximity).
      5. Free classifier VRAM before returning.
    """
    raise NotImplementedError("Real inference not wired yet — set GUARDIAN_MOCK=1")
