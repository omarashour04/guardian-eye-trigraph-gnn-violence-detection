"""
trigraph_v9_ntu_diagnostics.py
Guardian Eye - V9  |  Post-hoc Diagnostics (LOCAL, inference only)
Dataset : NTU CCTV-Fights

Design note
-----------
This is a LOCAL script (no Modal). It IMPORTS the model + dataset directly from
trigraph_v9_ntu_train so the diagnostic architecture is, by construction,
identical to the checkpoints that train.py produced (mean-over-heads GATv2).
This mirrors the RLVS leakage-analysis scripts in RLVS/scripts/ which also
import the V9 modules rather than re-defining the network.

Runs five diagnostics over the trained NTU experiments and writes CSV/JSON to
OUTPUT_DIR (default: ntu/train_output/runs_ntu/diagnostics/):

  1. gate_weights        - per-sample QGF gate weights on E_full_qgf (+ static
                           weights from E_full_lw). Checks VideoMAE dominance.
  ...
  4. error_analysis      - TP/TN/FP/FN breakdown with per-clip GQS + source.
  5. source_shortcut     - NTU-specific: is the model exploiting a source cue
                           (CCTV vs Mobile) or a quality shortcut, rather than
                           genuine fight content? Reports quality-alone AUC and
                           source base-rate per split + per source.

Usage
-----
  python trigraph_v9_ntu_diagnostics.py            # run all
  python trigraph_v9_ntu_diagnostics.py --only gate_weights
  python trigraph_v9_ntu_diagnostics.py --only calibration error_analysis
"""
...
# Guarantees the diagnostic V9Model matches the checkpoints exactly.
TRAIN = importlib.import_module("trigraph_v9_ntu_train")
...
NTUDataset = TRAIN.NTUDataset
V9Model    = TRAIN.V9Model
collate_v9 = TRAIN.collate_v9
seed_everything = TRAIN.seed_everything

# (streams, fusion_mode, extra_kwargs) per experiment - mirrors train.py.
EXP_CFG = {
...
def _load_split(split_name: str, split_df):
    """Return (npz_paths, labels) for a split. NTU split CSV id column = clip_id."""
...
def _build_model_for_exp(exp_id: str, device):
    if exp_id not in EXP_CFG:
        raise ValueError(f"Unknown experiment id: {exp_id}")
    active_streams, fusion_mode, extra = EXP_CFG[exp_id]
    model = V9Model(
        active_streams=active_streams, fusion_mode=fusion_mode,
...
def _compute_ece(y_true, y_prob, n_bins: int = 15) -> float:
    bins, ece, n_total = np.linspace(0.0, 1.0, n_bins + 1), 0.0, len(y_true)
    for lo, hi in zip(bins[:-1], bins[1:]):
        mask = (y_prob >= lo) & (y_prob < hi)
        if mask.sum() == 0:
            continue
        ece += mask.sum() / n_total * abs(y_true[mask].mean() - y_prob[mask].mean())
    return float(ece)
...
def diag_gate_weights(split_df, device):
    ...
    model = _build_model_for_exp("E_full_qgf", device)
    _load_checkpoint("E_full_qgf", model, device)
    model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="E_full_qgf", unit="batch"):
            ...
            logits, gates = model(bdev, return_gates=True)
            probs  = torch.sigmoid(logits).cpu().numpy()
            labels = bdev["label"].cpu().numpy().astype(int)
            g = gates.cpu().numpy()  # [B,4] order (skel, int, obj, vit)
            for i in range(len(probs)):
                rows.append({
                    ...
                    "true_label": int(labels[i]), "pred_prob": float(probs[i]),
                })
    ...
    print("\n  Mean gate weights by source:")
    print(gw.groupby("source")[cols].mean().round(4).to_string())
    ...
    try:
        model_lw = _build_model_for_exp("E_full_lw", device)
    ...
    print(f"  Saved -> {DIAG_DIR / 'gate_weights_E_full_qgf.csv'}")
...
def diag_gqs_stratified(split_df, device):
    ...
    if n_bins < 2:
        ...
        n_bins = test_gqs["q_bucket"].nunique()
    if n_bins < 2:
        print("  GQS composite has no variance; skipping.")
        return
    ...
    def run_subset(model, idxs):
        loader = DataLoader(Subset(test_ds, idxs), batch_size=64, shuffle=False,
                            num_workers=0, pin_memory=True, collate_fn=collate_v9)
        yt, yp = [], []
        for batch in loader:
            bdev = {k: v.to(device) if isinstance(v, torch.Tensor) else v
    ...
    for exp in ("E_full_qgf", "E_full_lw"):
        try:
            m = _build_model_for_exp(exp, device)
    ...
    for bucket, grp in test_gqs.groupby("q_bucket", observed=True):
        idxs = [cid_to_idx[c] for c in grp["clip_id"].tolist() if c in cid_to_idx]
        if not idxs:
            continue
...
def diag_calibration(split_df, device):
    ...
    print("=" * 70)
    EXP_ID = "E_full_qgf"
    model = _build_model_for_exp(EXP_ID, device)
    ...
    def collect_logits(split_name):
        ...
        loader = _make_loader(NTUDataset(paths, labels))
        lg, lb = [], []
        with torch.no_grad():
    ...
    def report(y, p, thr, tag):
        ...
        try:
            ...
            auc = float("nan")
        ece = _compute_ece(y, p, 15)
        print(f"  {tag:<30} F1={f1:.4f} AUC={auc:.4f} Acc={acc:.4f} "
              f"ECE={ece:.4f} thr={thr:.2f}")
    ...
    with open(str(DIAG_DIR / "calibration_results.json"), "w") as f:
        json.dump({"experiment": EXP_ID, "optimal_temperature": T_opt,
                   "val_nll_before": float(nll(1.0)),
                   "val_nll_after": float(nll(T_opt)),
                   "cal_threshold": best_thr,
                   "pre_calibration": pre_m, "post_calibration": post_m},
                  f, indent=2)
...
def diag_error_analysis(split_df, device):
    print("\n" + "=" * 70)
    print("4. ERROR ANALYSIS  (E_full_qgf TP/TN/FP/FN with GQS + source)")
    print("=" * 70)
    gqs_lookup = None
    if GQS_CSV.exists():
    ...
    test_paths, test_labels = _load_split("test", split_df)
    test_loader = _make_loader(NTUDataset(test_paths, test_labels))

    rows = []
    with torch.no_grad():
        for batch in tqdm(test_loader, desc=f"{EXP_ID}", unit="batch"):
            bdev = {k: v.to(device) if isinstance(v, torch.Tensor) else v
                    for k, v in batch.items()}
            ...
            for i in range(len(probs)):
                cid  = batch["clip_id"][i]
                p    = float(probs[i]); lbl = int(labels[i]); pred = int(p >= THR)
                seg  = ("TP" if lbl == 1 and pred == 1 else
                        "TN" if lbl == 0 and pred == 0 else
                        "FP" if lbl == 0 and pred == 1 else "FN")
                g = (gqs_lookup.loc[cid] if (gqs_lookup is not None
                     and cid in gqs_lookup.index) else None)
                rows.append({
                    "clip_id": cid, "source": batch["source"][i],
                    "true_label": lbl, "pred_prob": round(p, 6),
                    "pred_label": pred, "segment": seg,
    ...
    for seg in ["TP", "TN", "FP", "FN"]:
        ...
        print(f"  {seg} (n={len(sub)}):" +
              ("" if len(sub) == 0 else
               "  " + "  ".join(f"{c}={sub[c].mean():.3f}" for c in qcols)))
    DIAG_DIR.mkdir(parents=True, exist_ok=True)
...
# ==============================================================================
# 5. source_shortcut  (NTU-specific, read-only data audit - no model)
# ==============================================================================
def diag_source_shortcut(split_df, device):
    print("\n" + "=" * 70)
    print("5. SOURCE / QUALITY SHORTCUT AUDIT  (read-only, no model)")
    print("=" * 70)
    if not GQS_CSV.exists():
        print(f"  GQS summary not found: {GQS_CSV}; skipping.")
        return
    g = pd.read_csv(str(GQS_CSV))
    if "split" not in g.columns or "label" not in g.columns:
        meta = split_df[["clip_id", "split", "label", "source"]]
        g = g.merge(meta, on="clip_id", how="left", suffixes=("", "_s"))
    g["gqs_composite"] = 0.4 * g["q_skel"] + 0.4 * g["q_int"] + 0.2 * g["q_po"]

    qcols = [c for c in ["q_skel", "q_int", "q_obj", "q_po", "valid_ratio",
                         "gqs_composite"] if c in g.columns]

    print("\n  Per-split positive rate (should be ~0.5 - NTU is balanced):")
    rows = []
    for sp in ["train", "val", "test"]:
        sub = g[g["split"] == sp]
        if len(sub) == 0:
            continue
        pos_rate = float((sub["label"] == 1).mean())
        row = {"split": sp, "n": len(sub), "pos_rate": round(pos_rate, 4)}
        for c in qcols:
            try:
                auc = float(roc_auc_score(sub["label"], sub[c]))
            except ValueError:
                auc = float("nan")
            row[f"auc_{c}"] = round(auc, 4)
        rows.append(row)
        print(f"    {sp:<6} n={len(sub):5d}  pos_rate={pos_rate:.4f}  "
              f"quality-alone AUC composite="
              f"{row.get('auc_gqs_composite', float('nan'))}")

    # Source base-rate: does a source correlate with the label? (within-video
    # negatives mean each source contributes both pos and neg, so this SHOULD
    # be near 0.5 - a large skew would be a source-shortcut risk.)
    if "source" in g.columns:
        print("\n  Per-source positive rate (within-video negatives -> expect ~0.5):")
        src_rows = []
        for src, sub in g.groupby("source"):
            pr = float((sub["label"] == 1).mean())
            src_rows.append({"source": src, "n": len(sub), "pos_rate": round(pr, 4)})
            print(f"    {src:<8} n={len(sub):5d}  pos_rate={pr:.4f}")
        pd.DataFrame(src_rows).to_csv(
            str(DIAG_DIR / "source_base_rate.csv"), index=False)

    DIAG_DIR.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(rows).to_csv(str(DIAG_DIR / "shortcut_audit.csv"), index=False)
    print(f"  Saved -> {DIAG_DIR / 'shortcut_audit.csv'}")
...
ALL_DIAGS = {
    ...
    "source_shortcut": diag_source_shortcut,
}
...
def main(args):
    ...
    if not SPLIT_CSV.exists():
        raise RuntimeError(f"split_ntu.csv not found at {SPLIT_CSV}. Run training first.")
    ...
    for name in to_run:
        if name not in ALL_DIAGS:
            print(f"  WARN: unknown diagnostic '{name}', skipping.")
            continue
        try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NTU CCTV-Fights V9 Diagnostics (local)")
    parser.add_argument("--only", nargs="+", default=None,
                        help="Subset of diagnostics to run "
                             "(gate_weights gqs_stratified calibration "
                             "error_analysis source_shortcut)")
