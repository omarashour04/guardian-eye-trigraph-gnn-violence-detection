...
Identical to the RLVS Phase 2 script except:
  - paths point at the NTU preproc/finetune output dirs
  - split CSV is split_ntu.csv (id column: clip_id, one row per WINDOW)
  - dataset class renamed NTUVideoDataset (behaviour identical)
NPZ schema, VideoMAE recipe, Stage A/B logic are byte-identical to RLVS so the
768-d embeddings are produced the same way for cross-dataset comparability.

Stage A - Fine-tune VideoMAE-Base (Kinetics-400 pretrained) as a standalone
  binary violence classifier. Best checkpoint -> OUTPUT_DIR/videomae_best.pt.
  Records standalone accuracy as ablation experiment C.
Stage B - Embedding extraction. Mean-pool final ViT hidden states -> 768-d
  vector, written back into every NPZ (all splits) as vit_embedding.

NTU is balanced 1:1 by construction (negatives downsampled to positives per
split in Phase 1), so pos_weight ~1.0 - still computed dynamically.

Usage
-----
  python trigraph_v9_ntu_videomae.py
  python trigraph_v9_ntu_videomae.py --skip-finetune    # Stage B only
  python trigraph_v9_ntu_videomae.py --force-finetune   # redo Stage A
  python trigraph_v9_ntu_videomae.py --force-reembed    # redo all of Stage B
"""
...
class ViTCFG:
    ...
    batch_size:      int   = 8     # RTX 3090 24 GB shared - smoke test confirmed bs=8+ckpt fits
    epochs:          int   = 50
    patience:        int   = 10
    ...
    warmup_epochs:   int   = 5
    accum_steps:     int   = 8     # effective batch = 8 * 8 = 64
    grad_clip:       float = 1.0
    label_smoothing: float = 0.05
    # RWF recipe (balanced data) - NTU is also balanced 1:1 by construction
    ...
    # pin_memory off by default: when SHARING the GPU with another job, the pinned
    # allocator can stall on Windows WDDM. Off costs a few %% transfer speed only.
    pin_memory:      bool  = False
    # Gradient checkpointing trades compute for memory. If the run hangs at the
    # FIRST loss.backward() (heartbeat stops after "forward pass done"), reentrant
    # checkpointing is the likely deadlock -> set this False to disable it. With
    # checkpointing off you may need a smaller batch_size to fit VRAM.
    ...
    def __init__(self, npz_paths: List[Path], labels: List[int],
        ...
        for path, lbl in tqdm(zip(npz_paths, labels),
            ...
            try:
                ...
                self.samples.append({
                    "frames": d["frames_vit"],   # uint8 [T, H, W, 3]
                    "label":  float(lbl),
                    "path":   str(path),
                })
...
def build_videomae_model(model_name: str, embed_dim: int, dropout: float):
    ...
    if vcfg.grad_checkpointing:
        # Prefer non-reentrant checkpointing: the reentrant default can deadlock
        # in the first backward on some PyTorch+CUDA builds. Fall back to the
        # plain call on older transformers that lack the gradient_checkpointing_kwargs arg.
        try:
            base.gradient_checkpointing_enable(
                gradient_checkpointing_kwargs={"use_reentrant": False})
            print("  Gradient checkpointing: ON (non-reentrant)")
        except TypeError:
            base.gradient_checkpointing_enable()
            print("  Gradient checkpointing: ON (reentrant - older transformers)")
    else:
...
def main(args) -> None:
    ...
    if skip_stage_a:
        ...
        train_ds = NTUVideoDataset(train_paths, train_labels, augment=True)
        val_ds   = NTUVideoDataset(val_paths,   val_labels,   augment=False)

        # Dynamic pos_weight - NTU is balanced 1:1 so pw ~1.0
        ...
        train_loader = DataLoader(train_ds, batch_size=vcfg.batch_size,
                                  ...
                                  pin_memory=vcfg.pin_memory, collate_fn=collate_fn)
        ...
        for epoch in range(start_epoch, vcfg.epochs + 1):
            ...
            for step, batch in pbar:
                # First-batch heartbeat: a silent 0/151 hang gives no clue WHERE
                # it stalled. On the very first step of the run, print between each
                # native CUDA call so a deadlock is localised immediately. Costs
                # nothing after step 0.
                hb = (epoch == start_epoch and step == 0)
                if hb:
                    print("  [hb] batch 0 fetched from loader", flush=True)
                pv  = batch["pixel_values"].to(DEVICE)  # [B, T, 3, H, W]
                lbl = batch["label"].to(DEVICE)
                if hb:
                    if torch.cuda.is_available():
                        torch.cuda.synchronize()
                    print("  [hb] batch 0 moved to GPU", flush=True)
                ...
                if hb:
                    if torch.cuda.is_available():
                        torch.cuda.synchronize()
                    print("  [hb] forward pass done", flush=True)
                loss   = smooth_bce(logits, lbl_m) / vcfg.accum_steps
                loss.backward()
                if hb:
                    if torch.cuda.is_available():
                        torch.cuda.synchronize()
                    print("  [hb] backward pass done", flush=True)
        ...
        test_ds     = NTUVideoDataset(test_paths, test_labels, augment=False)
    ...
    class LazyNPZDataset(Dataset):
        """Reads NPZs from disk on demand - avoids loading all clips at once."""
    ...
    full_loader = DataLoader(full_ds, batch_size=vcfg.batch_size,
                             shuffle=False, num_workers=vcfg.num_workers,
                             pin_memory=vcfg.pin_memory, collate_fn=collate_lazy)
        ...
        for batch in tqdm(full_loader, desc="Extracting+writing embeddings",
            ...
            for path_str, emb in zip(batch["paths"], embs):
                ...
                try:
                    ...
                    tmp.replace(npz_path)  # atomic overwrite on Windows
                    written += 1
                except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NTU CCTV-Fights V9 VideoMAE")
