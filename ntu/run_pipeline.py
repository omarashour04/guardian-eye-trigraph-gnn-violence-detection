"""
run_pipeline.py
Guardian Eye - V9  |  NTU CCTV-Fights end-to-end orchestrator

Runs the phases in order, stopping immediately if any phase fails:
  1. trigraph_v9_ntu_preprocess.py    (Phase 1 - windowing + preprocessing)
  2. trigraph_v9_ntu_videomae.py      (Phase 2 - VideoMAE fine-tune + embeddings)
  3. trigraph_v9_ntu_train.py         (Phase 3 - graph training + ablation)
  4. trigraph_v9_ntu_diagnostics.py   (Phase 4 - post-hoc diagnostics) [OPT-IN]
...
-----
  ...
  python run_pipeline.py --plan-only            # only window + write split CSV, then stop
...
-----
  ...
  - --plan-only forwards to the preprocess script and stops the pipeline after
    Phase 1 (no YOLO, no NPZ) so you can inspect the clip plan / class balance.
...
PHASES = [
    ("preprocess",  "trigraph_v9_ntu_preprocess.py",  "Phase 1 - Preprocessing"),
    ("videomae",    "trigraph_v9_ntu_videomae.py",    "Phase 2 - VideoMAE"),
    ("train",       "trigraph_v9_ntu_train.py",        "Phase 3 - Training"),
    ("diagnostics", "trigraph_v9_ntu_diagnostics.py",  "Phase 4 - Diagnostics"),
]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="NTU CCTV-Fights V9 pipeline orchestrator")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Run only Phase 1 in --plan-only mode (window videos + write split "
             "CSV, no YOLO), then stop. Lets you inspect the clip plan first.")
    args = parser.parse_args()

    # --plan-only: short-circuit to a single preprocess run and stop.
    if args.plan_only:
        print(f"{'='*70}")
        print("Guardian Eye V9 - NTU pipeline (PLAN-ONLY)")
        print(f"  Interpreter : {sys.executable}")
        print(f"{'='*70}", flush=True)
        return run_phase(PHASES[0][1], PHASES[0][2] + " (plan-only)",
    ...
    print(f"{'='*70}")
    print("Guardian Eye V9 - NTU CCTV-Fights pipeline")
    print(f"  Interpreter : {sys.executable}")
