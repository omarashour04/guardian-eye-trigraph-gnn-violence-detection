...
Stream 1 — Enhanced Multi-Input STGCN
  Input    : skeleton [T, M, V, 3] -> joint+bone+joint_motion+bone_motion -> C=12
  Spatial  : 3 x GraphTemporalConv [12->64->64->128]
             adaptive adjacency A = A_fixed(COCO-17) + softmax(A_learned)
  Pooling  : learned attention over M persons
  Output   : e_skel [B, 128]
...
Stream 3 — Object/PO (FrameGINE + BiGRU)
  Input    : obj_nodes, obj_node_mask, po_edges, po_edge_mask
  Output   : e_obj [B, 128]
...
Fusion    — QualityGatedFusion (E-QGF) or LearnedWeightFusion (E-LW)
  Gate input: gqs [B, 5] -> MLP(5->32->n) -> softmax -> stream weights
...
─────────────────────
  ...
  D  — STGCN + interaction + object/PO
  E_full_qgf       — All 4 streams, QualityGatedFusion
  E_full_lw        — All 4 streams, LearnedWeightFusion (ablation control)
  E_full_qgf_fixed — QGF + entropy reg + temperature annealing

RLVS is ~1:1 balanced (1,000 fight / 1,000 non-fight).
pos_weight is computed dynamically from train labels.
No hard-negative exclusions applied (no known OOD clips yet;
add clip_id stems to cfg.hard_negative_clip_ids after error analysis).

Resumability (on by default)
────────────────────────────
  - Finished experiments (those with test_metrics.json) are skipped and their
    result reloaded into the comparison table.
  - An experiment killed mid-training writes last.pt every epoch (full model+
    optimizer+scheduler+patience state); re-running continues from the next
    epoch. last.pt is deleted once the experiment completes.
  Just re-run the same command after any interruption.
  Use --no-resume to force every experiment to retrain from scratch.

Usage
─────
  python trigraph_v9_rlvs_train.py
  python trigraph_v9_rlvs_train.py --no-resume   # retrain everything from scratch
"""
...
FINETUNE_DIR = Path(r"C:\Violence detection\ashour\fresh\RLVS\finetune_output")
OUTPUT_DIR   = Path(r"C:\Violence detection\ashour\fresh\RLVS\train_output")

CACHE_DIR = PREPROC_DIR  / "cache_v9"
SPLIT_CSV = PREPROC_DIR  / "split_rlvs.csv"
GQS_CSV   = PREPROC_DIR  / "gqs_summary_rlvs.csv"
RUN_DIR   = OUTPUT_DIR   / "runs_rlvs"
...
class CFG:
    ...
    # pos_weight is computed dynamically from train labels (RLVS is ~1:1)
    batch_size:      int   = 64
    ...
    # Hard-negative exclusion (clip_id stems, RLVS-specific)
...
class V9Model(nn.Module):
    ...
    def forward(self, batch: Dict) -> torch.Tensor:
        ...
        fused = streams_out[0] if len(streams_out) == 1 \
            else self.fusion(streams_out, batch["gqs"])
...
def main(args) -> None:
    ...
    if len(test_paths) == 0:
        raise RuntimeError(
            "No test split found in split_rlvs.csv. "
            "Check that Phase 1 completed successfully."
        )
    print(f"Train={len(train_paths)}  Val={len(val_paths)}  "
          f"Test={len(test_paths)}")

    train_ds = RLVSDataset(train_paths, train_labels)
    val_ds   = RLVSDataset(val_paths,   val_labels)
    test_ds  = RLVSDataset(test_paths,  test_labels)

    # Dynamic pos_weight — RLVS is ~1:1 so this will be ~1.0
    ...
    EXPERIMENTS = [
        ...
        ("E_full_lw",          ("skeleton", "interaction", "object", "vit"),
         "lw",  {}),
        # E_full_qgf_fixed: QGF with entropy regularisation + temperature
        # annealing (2.0->0.5 over epochs) to maximise gate spread. Matches the
        # RWF ablation table for paper parity.
    ...
    for exp_id, active_streams, fmode, ovr in EXPERIMENTS:
        try:
            ...
            # Resume: skip if test_metrics.json already exists
            ...
            history      = []
            start_epoch  = 1
            last_path    = exp_dir / "last.pt"   # full-state, written every epoch

            # ── Resume this experiment from its last.pt if present ────────
            # Lets a single experiment that died mid-training (e.g. at epoch 40)
            # continue from epoch 41 with exact optimizer/scheduler/patience
            # state, instead of restarting from scratch. Disabled under --no-resume.
            ...
            for epoch in range(start_epoch, cfg.epochs + 1):
                ...
                # Full-state "last" checkpoint EVERY epoch — the resume point.
                # Atomic tmp->rename so a kill mid-write cannot corrupt it.
                ...
                # Persist per-epoch history too, so it survives a kill.
            ...
            y_tt, y_pt = evaluate(model, test_loader, DEVICE)
            ...
            # Add calibrated metrics and dual threshold metrics
            ...
            # Experiment complete — drop the per-epoch resume checkpoint so a
            # later --no-resume run doesn't mistakenly resume a finished one.
            if last_path.exists():
                try:
                    last_path.unlink()
                except Exception:
    ...
    results_csv = RUN_DIR / "experiment_comparison_rlvs.csv"
    results_df.to_csv(str(results_csv), index=False)

    print(f"\n{'='*70}")
    print("RLVS ABLATION COMPARISON")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RLVS V9 Training")
